...
export async function sheetsRequest(path, init = {}) {
  ...
  if (!res.ok) {
    ...
    if (res.status === 429) {
      ...
      return sheetsRequest(path, init);
    }
    throw new Error(`Sheets API ${res.status}: ${errorText}`);
  }
  return res.json();
}
...
export const appendRow = (range, values) => 
  sheetsRequest(`/values/${encodeURIComponent(range)}:append?valueInputOption=USER_ENTERED&insertDataOption=INSERT_ROWS`, {
    method: 'POST',
    body: JSON.stringify({ values: [values] })
  });

export const updateRow = (range, values) => 
  sheetsRequest(`/values/${encodeURIComponent(range)}?valueInputOption=USER_ENTERED`, {
    method: 'PUT',
    body: JSON.stringify({ values: [values] })
  });
